...
def create_user(
    ...
    """
    Creates a user or client and assigns roles.
    Generates INVITE TOKEN for first-time onboarding.
    Role 5 = Client → stored in clients table.
    """
    ...
    if is_client:
        ...
        if existing_client:
            invite_token = create_invite_token(existing_client.id)
            return {
                "id": existing_client.id,
                "email": existing_client.email,
                "message": "Client already exists",
        ...
        invite_token = create_invite_token(client.id)
    ...
    # ✅ Generate INVITE TOKEN instead of temp token
